function varargout = editADVMProc(varargin)
% EDITADVMPROC MATLAB code for editADVMProc.fig
%      EDITADVMPROC, by itself, creates a new EDITADVMPROC or raises the existing
%      singleton*.
%
%      H = EDITADVMPROC returns the handle to a new EDITADVMPROC or the handle to
%      the existing singleton*.
%
%      EDITADVMPROC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in EDITADVMPROC.M with the given input arguments.
%
%      EDITADVMPROC('Property','Value',...) creates a new EDITADVMPROC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before editADVMProc_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to editADVMProc_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help editADVMProc

% Last Modified by GUIDE v2.5 12-Jan-2015 15:30:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @editADVMProc_OpeningFcn, ...
                   'gui_OutputFcn',  @editADVMProc_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before editADVMProc is made visible.
function editADVMProc_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to editADVMProc (see VARARGIN)
...
%         'String',   advmParamStruct.NumberOfCells);
    set(handles.beam_popupmenu,...
        'Value',    ...
        find(strcmp(advmParamStruct.BeamNumber,BeamNumber)));
    set(handles.movAvgSpan_edit,...
        'String',   advmParamStruct.MovingAverageSpan);
    set(handles.backscatter_popupmenu,...
        'Value',    ...
        find(strcmp(advmParamStruct.BSValues,BSValues)));
    set(handles.minCellMidPoint_edit,...
        'String',   advmParamStruct.RMin);
    set(handles.maxCellMidPoint_edit,...
        'String',   advmParamStruct.RMax);
    set(handles.minVbeam_edit,...
        'String',   advmParamStruct.MinVbeam);
    set(handles.nearField_checkbox,...
        'Value',    advmParamStruct.NearField);
    
    
    if isempty(advmParamStruct.NumberOfCells)
        set(handles.minCells_popupmenu,...
            'String',   '-',...
            'Value',     1  ...
            );
        advmParamStruct.MinCells = NaN;
    else
        set(handles.minCells_popupmenu,...
            'String',num2cell(1:advmParamStruct.NumberOfCells),...
            'Value', advmParamStruct.MinCells);
    end
    
    if strcmp(advmParamStruct.BSValues,'Amp')
        set(handles.intenScaleFactor_edit,...
            'enable',   'on',...
            'String',   advmParamStruct.IntenScale);
        set(handles.intenScaleFactor_text,...
            'enable',   'on');
    end
    
    set(handles.removeMinWCB_checkbox,...
        'Value',advmParamStruct.RemoveMinWCB);
...
% Choose default command line output for editADVMProc
...
% UIWAIT makes editADVMProc wait for user response (see UIRESUME)
uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = editADVMProc_OutputFcn(hObject, eventdata, handles) 
...
if isnan(advmParamStruct.NumberOfCells)
    set(handles.minCells_popupmenu,...
        'String',   '-',...
        'Value',     1  ...
        );
...
% set(hObject,'String',num2str(IntenScale));
